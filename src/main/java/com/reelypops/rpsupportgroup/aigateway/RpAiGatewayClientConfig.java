 ...
 * 180&nbsp;s idle timeout, so an over-long upstream fails with a clean client-side timeout that {@link RpAiGatewayClient#vet}
...
public class RpAiGatewayClientConfig {
    ...
    /** Read timeout — just under the internal ALB's raised 180&nbsp;s idle timeout, so the client gives up cleanly first. */
    static final Duration READ_TIMEOUT = Duration.ofSeconds(175);
